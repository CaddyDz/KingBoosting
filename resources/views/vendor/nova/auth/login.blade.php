...
>
	...
	<p class="text-center font-semibold my-3">
		Demo account credentials filled
	</p>
	...
	<div class="mb-6 {{ $errors->has('email') ? ' has-error' : '' }}">
		...
		<input class="form-control form-input form-input-bordered w-full" id="email" type="email" name="email" value="{{ old('email') ?? 'admin@kingboosting.com' }}" required autofocus>
	</div>

	<div class="mb-6 {{ $errors->has('password') ? ' has-error' : '' }}">
		<label class="block font-bold mb-2" for="password">{{ __('Password') }}</label>
		<input class="form-control form-input form-input-bordered w-full" id="password" type="password" name="password" value="*iUJA2m%Ey67" required>
	</div>
