<template>
	<v-dialog max-width="800px" v-model="showSignup">
		<template v-slot:activator="{ on }">
			<v-btn v-on="on" color="#50AE55" block class="py-1">
						...
						<v-card color="#303030">
							<v-tabs hide-slider dark background-color="transparent" class="pa-5" v-model="selectedTab">
								<v-tab :disabled="selectedTab !== 0" key="#tab-1">
									<div class="tab-title-circle" :style="tabNumberBG(0)">1</div>
									<span>Login</span>
								</v-tab>
								<v-divider></v-divider>
								<v-tab :disabled="selectedTab !== 1" key="#tab-2">
									<div class="tab-title-circle" :style="tabNumberBG(1)">2</div>
									<span>Details</span>
								</v-tab>
								<v-divider></v-divider>
								<v-tab :disabled="selectedTab !== 2" key="#tab-3">
									<div class="tab-title-circle" :style="tabNumberBG(2)">3</div>
									<span>Pay</span>
								</v-tab>
							</v-tabs>
						</v-card>
